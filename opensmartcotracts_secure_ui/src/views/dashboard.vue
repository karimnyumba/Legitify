    ...
    <v-row>
      <v-hover>
        <v-col v-for="doc in documents" :key="doc.time" cols="2" sm="2" md="2">
          <v-card
            @click="openDoc(doc.cidValue)"
            class="col-card"
            url="http://localhost:8080/ipfs/QmYBySzGhc8hJYvRSfFC9fd9q6TVwMEhAtx1DmYiWq44SP"
          >
            ...
            <div class="ps-3 ms-3">
              <h6 class="folder-texti">
                ...
                ><br />
                <span class="pe-1">Stored:</span
                >{{ new Date(parseInt(doc.time)).toLocaleTimeString() }}
              </h6>
            </div>
          </v-card>
        </v-col>
      ...
      <v-table>
        ...
        <tbody>
          <tr v-for="doc in documents" :key="doc.time">
            ...
            <td>
              <span>{{
                new Date(parseInt(share.time)).toLocaleTimeString()
              }}</span
              ><span class="ps-2">{{
                new Date(parseInt(share.time)).toLocaleDateString()
              }}</span>
            </td>
...
export default {
  ...
  methods: {
    ...
    async storeFile(e) {
      ...
      const storedResponse = await contract.store(
        cid.path,
        time,
        comment,
        docName
